...
function addPriceSummary(pptx: PptxGenJS, slide: PptxGenJS.Slide, proposal: ProposalDocument, accent: string, fonts: ReturnType<typeof fontProfileForBrand>) {
  ...
  const w = 4.15;
  const h = 2.05;
  slide.addShape(pptx.ShapeType.roundRect, { x, y, w, h, rectRadius: 0.16, fill: { color: '19272D', transparency: 16 }, line: { color: layout === 'onepager_discount' ? accent : '32434B', transparency: 18, width: 0.8 } });
  ...
  if (layout === 'onepager_discount' && totals.listMonthlyTotal > totals.monthlyTotal) {
    addAdaptiveText(slide, `Полная ${formatMoney(totals.listMonthlyTotal)}`, { x: x + 0.22, y: y + 0.96, w: w - 0.44, h: 0.18, ...textStyle(fonts, 'caption'), color: COLORS.muted, margin: 0 }, { maxLines: 1 });
  ...
  if (totals.oneTimeTotal) addAdaptiveText(slide, `Разово: ${formatMoney(totals.oneTimeTotal)}`, { x: x + 0.22, y: y + 1.63, w: w - 0.44, h: 0.18, ...textStyle(fonts, 'caption'), color: COLORS.white, margin: 0 }, { maxLines: 1 });
}

export function renderOnePager(pptx: PptxGenJS, proposal: ProposalDocument, assets: PresentationAssets) {
  ...
  addAdaptiveText(slide, `РЕШЕНИЕ ДЛЯ ${proposal.client.name.toUpperCase()}`, { x: SLIDE.marginX, y: 1.36, w: 6.95, h: 0.72, ...textStyle(fonts, 'title'), color: COLORS.white, margin: 0, valign: 'top' }, { maxLines: 2, minFontSize: 14 });
  addAdaptiveText(slide, proposal.cover.subtitle, { x: SLIDE.marginX, y: 2.17, w: 6.95, h: 0.34, ...textStyle(fonts, 'body'), color: 'D6E0E4', margin: 0, valign: 'top' }, { maxLines: 2, minFontSize: 8 });
  if (manager) {
    ...
    addManagerAvatar(pptx, slide, manager.photoDataUrl, assets.uiIcons?.userPlaceholder, box.x + 0.2, box.y + 0.2, accent, false);
  ...
  addPriceSummary(pptx, slide, proposal, accent, fonts);

  const solutionBox = { x: 0.58, y: 2.72, w: 7.56, h: 2.05 };
  slide.addShape(pptx.ShapeType.roundRect, { ...solutionBox, rectRadius: 0.16, fill: { color: '19272D', transparency: 16 }, line: { color: '32434B', transparency: 18, width: 0.8 } });
  ...
  products.forEach((product, index) => {
    ...
    addAdaptiveText(slide, product.shortName, { x: x + visualSize + 0.14, y, w: 3.36 - visualSize, h: 0.2, ...textStyle(fonts, 'caption'), color: COLORS.white, margin: 0, valign: 'top', wrap: false }, { singleLine: true, minFontSize: 6 });
    addAdaptiveText(slide, product.shortValue, { x: x + visualSize + 0.14, y: y + 0.27, w: 3.36 - visualSize, h: 0.31, ...textStyle(fonts, 'caption'), color: 'B6C4CA', margin: 0, valign: 'top' }, { maxLines: 2, minFontSize: 6 });
  });

  const benefitsBox = { x: 0.58, y: 4.98, w: 7.56, h: 1.55 };
  slide.addShape(pptx.ShapeType.roundRect, { ...benefitsBox, rectRadius: 0.16, fill: { color: '19272D', transparency: 16 }, line: { color: '32434B', transparency: 18, width: 0.8 } });
  ...
  benefits.forEach((benefit, index) => {
    ...
    addAdaptiveText(slide, benefit.title, { x: x + 0.32, y: 5.52, w: 1.85, h: 0.2, ...textStyle(fonts, 'caption'), color: COLORS.white, margin: 0, valign: 'top' }, { maxLines: 2, minFontSize: 6 });
    addAdaptiveText(slide, benefit.description, { x: x + 0.32, y: 5.75, w: 1.85, h: 0.5, ...textStyle(fonts, 'caption'), color: 'B6C4CA', margin: 0, valign: 'top' }, { maxLines: 3, minFontSize: 5.5 });
  });
  ...
  if (caseItem) {
    ...
    slide.addShape(pptx.ShapeType.roundRect, { ...box, rectRadius: 0.16, fill: { color: '19272D', transparency: 16 }, line: { color: '32434B', transparency: 18, width: 0.8 } });
  ...
  addAdaptiveText(slide, legalTerms.shortVersion, { x: SLIDE.marginX, y: 7.24, w: 11.89, h: 0.18, ...textStyle(fonts, 'caption'), color: 'AEBEC5', margin: 0, wrap: false }, { singleLine: true, minFontSize: 5.5 });
  addNotes(slide);
}
