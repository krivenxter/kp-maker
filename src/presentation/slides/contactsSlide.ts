...
export function renderContactsSlide(pptx: PptxGenJS, proposal: ProposalDocument, assets: PresentationAssets) {
  const slide = pptx.addSlide();
  const accent = accentForBrand(proposal.client.brandId);
  const fonts = fontProfileForBrand(proposal.client.brandId);
  addBackground(pptx, slide, 'dark', assets.finalBackground);
  addBrandHeader(pptx, slide, 'dark', assets.logoLight, proposal.client.name, accent, fonts, proposal.client.site);
  addAdaptiveText(slide, 'ОБСУДИМ ЗАПУСК', { x: 0.76, y: 2.0, w: 6.2, h: 0.82, ...textStyle(fonts, 'display'), color: COLORS.white, margin: 0, wrap: false }, { singleLine: true, minFontSize: 22 });
  addAdaptiveText(slide, `Решения Calltouch для ${proposal.client.name}`, { x: 0.78, y: 2.96, w: 5.4, h: 0.42, ...textStyle(fonts, 'body'), color: 'C6D2D7', margin: 0 }, { maxLines: 2, minFontSize: 8 });
  ...
  if (manager) {
    ...
    addAdaptiveText(slide, manager.name, { x: contactX, y: box.y + 0.27, w: 2.8, h: 0.26, ...textStyle(fonts, 'heading'), color: COLORS.white, margin: 0, wrap: false }, { singleLine: true, minFontSize: 9 });
    addAdaptiveText(slide, manager.position, { x: contactX, y: box.y + 0.6, w: 2.8, h: 0.28, ...textStyle(fonts, 'caption'), color: COLORS.muted, margin: 0, valign: 'top' }, { maxLines: 2, minFontSize: 6.5 });
    ...
    addAdaptiveText(slide, manager.email, { x: contactTextX, y: box.y + 0.98, w: 2.73, h: 0.22, ...textStyle(fonts, 'caption'), color: COLORS.white, margin: 0, valign: 'middle', wrap: false }, { singleLine: true, minFontSize: 6 });
    addImageFit(slide, assets.uiIcons?.phone ?? '', { x: contactIconX, y: box.y + 1.285, w: 0.17, h: 0.17 }, 'contain');
    addAdaptiveText(slide, manager.phone, { x: contactTextX, y: box.y + 1.26, w: 2.73, h: 0.22, ...textStyle(fonts, 'caption'), color: COLORS.white, margin: 0, valign: 'middle', wrap: false }, { singleLine: true, minFontSize: 6 });
  }
  addNotes(slide);
}
